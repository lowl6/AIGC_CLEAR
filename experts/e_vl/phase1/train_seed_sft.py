"""任务二 Seed SFT 训练脚本（Qwen3-VL + LoRA）。
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="任务二 Seed SFT 训练")
    parser.add_argument(
        "--train_path",
        type=Path,
        default=Path("checkpoints/e_vl/seed_sft_data/train_seed_sft.jsonl"),
    )
    parser.add_argument(
        "--val_path",
        type=Path,
        default=Path("checkpoints/e_vl/seed_sft_data/val_seed_sft.jsonl"),
    )
    parser.add_argument(
        "--model_name_or_path",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=Path,
        default=Path("checkpoints/e_vl/seed_sft_lora"),
    )
...
def main() -> None:
    ...
    collator = Qwen3VLMultiImageCollator(processor=processor, max_length=args.max_length)
    ...
    print(f"[SeedSFT-任务二] 训练完成，输出目录: {args.output_dir}")
